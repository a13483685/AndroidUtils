...
public class ImageLoaderActivity extends AppCompatActivity implements AbsListView.OnScrollListener{
    ...
    private void initView() {
        ...
        mImageAdapter = new ImageAdapter(ImageLoaderActivity.this);
    ...
    private class ImageAdapter extends BaseAdapter{
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if(convertView ==null){
                convertView = mInflater.inflate(R.layout.photo_wall_item,parent,false);
            ...
            final String uri = getItem(position);//获取到网络连接
            if(!uri.equals(tag)){
                imageView.setImageDrawable(mDefaultBitmapDrawable);
            }
            if (mIsGridViewIdle && mCanGetBitmapFromNetWork) {
                imageView.setTag(uri);//只有在没有滑动的时候，才设置tag，bindBitmap。也就是在滑动的时候下载图片。这样可以防止图片加载错位
